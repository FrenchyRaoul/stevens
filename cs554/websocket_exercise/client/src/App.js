...
function App() {
  ...
  const changeRoom = (room) => {
    ...
    socketRef.current.emit("join_room", {name: state.name, room: room});
  }

  const changeRoomFuncFactory = (room) => {
    return () => changeRoom(room);
  }

  return (
    <div>
      {state.name && (<Navigation onClick={changeRoomFuncFactory}/>)}
      {state.name && (
        <div className='card'>
          <div className='render-chat'>
            <h1>{roomRef.current} Chat</h1>
            {renderChat()}
          </div>
      ...
      >
        <button type='submit'>run room test</button>
      </form>
    </div>
  );
}
