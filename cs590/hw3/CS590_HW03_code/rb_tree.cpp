...
void rb_tree::inorder_tree_walk(int* array, int* n, rb_tree_node* z)
{
    if (z->left != T_nil)
...
{
    int black_height_left;
    int black_height_right;

    if (x == T_nil)
        return 1;

    int left_adder = (x->left->color == rb_tree_color::RB_BLACK) ? 1 : 0;
    int right_adder = (x->right->color == rb_tree_color::RB_BLACK) ? 1 : 0;

    black_height_left = check_black_height(x->left) + left_adder;
    black_height_right = check_black_height(x->right) + right_adder;

    if (black_height_left != black_height_right)
        cout << "WARNING: black height is not balanced" << endl;
